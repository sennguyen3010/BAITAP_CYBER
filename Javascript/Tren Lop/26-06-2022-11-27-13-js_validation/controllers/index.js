...
document.querySelector('#btnThemSinhVien').onclick = function () {
    ...
    //.trim(): Loại bỏ khoảng trống đầu và cuối của 1 chuỗi,  ví dụ chuỗi         abc       : =>  abc
    var valid = true;
    //Kiểm tra rỗng 
    valid &= kiemTraRong(sv.maSinhVien,'#error_required_maSinhVien','Mã sinh viên') & kiemTraRong(sv.tenSinhVien,'#error_required_tenSinhVien','Tên sinh viên');

    //Kiểm tra độ dài
    valid &= kiemTraDoDai(sv.matKhau,'#err_matKhau_min_max_length','Mật khẩu',6,32);


    //Kiểm tra định dạng
    var ngaySinhTest = moment(ngaySinh).format('YYYY-MM-DD');
    valid &= kiemTraTatCaKyTu(sv.tenSinhVien,'#error_all_letter_tenSinhVien','Tên sinh viên') & kiemTraTatCaSo(sv.diemToan,'#err_diemToan','Điểm toán') & kiemTraTatCaSo(sv.diemLy,'#err_diemLy','Điểm lý') & kiemTraTatCaSo(sv.diemHoa,'#err_diemHoa','Điểm hoá') & kiemTraEmail(sv.email,'#err_email','Email') & kiemTraMatKhau(sv.matKhau,'#err_matKhau','Mật khẩu') & kiemTraNgayThangNam(ngaySinhTest,'#err_ngaySinh','Ngày sinh');

    //Kiểm tra giá trị
    valid &= kiemTraGiaTri(sv.diemToan,'#err_min_max_value_diemToan','Điểm toán',0,10) &  kiemTraGiaTri(sv.diemLy,'#err_min_max_value_diemLy','Điểm lý',0,10) & kiemTraGiaTri(sv.diemHoa,'#err_min_max_value_diemHoa','Điểm hoá',0,10);


    // var regex =   /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
    // ;

    // var ketQua = regex.test(sv.email);
...
function xoaSinhVienTheoMa(maSV) {
    ...
    for(var index = mangSinhVien.length - 1; index >= 0; index --) {
        ...
        var sinhVien = mangSinhVien[index];
        if(sinhVien.maSinhVien == maSV) { //nếu obj sinh viên trong mảng == với lại mã sinh viên được click thì lấy ra vị trí
            // viTriXoa = index;
            // break;
